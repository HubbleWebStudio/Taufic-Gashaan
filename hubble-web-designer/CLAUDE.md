...
├── hubble-web-designer/      ← Skillset: wd-arranque, wd-construccion, wd-refinamiento, wd-biblioteca, wd-auditoria
...
5. Preguntar qué etapa necesita e invocar el skill correspondiente:
   ...
   - **Construir sección con referencia visual (imagen o URL) o sin referencia** → skill `wd-construccion`
   - **Crear página/sección sin referencia visual** → skill `wd-prototipo` primero (HTML fiel sección x sección), luego `wd-construccion` para portar a Next.js
...
| `wd-biblioteca` | `wd-construccion` | puntual — si hay decisión de diseño abierta no resuelta por la referencia |
...
- **Referencia Obligatoria:** No se genera código de UI sin un análisis previo del insumo visual (imagen / URL). Sin referencia: activar `wd-prototipo` primero para generar el HTML de referencia.
